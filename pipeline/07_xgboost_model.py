...
Inputs:
    data/processed/ncaaw_players_features.csv   (from 06_archetype_classifier.py)
...
def load_historical_features(features_df: pd.DataFrame) -> pd.DataFrame:
    """
    Load raw NCAA files for years not already in features_df and return a
    combined lookup pool for match_ncaa_features.

    The raw files (from 01b_scrape_draft_class_only.py) have slightly different
    column names than the processed features CSV.  This function normalises them
    so match_ncaa_features can treat the combined pool uniformly.

    features_df is passed in so we know which years are already covered and
    don't need to be re-loaded from raw.
    """
    covered_years = set(
        features_df["most_recent_year"].dropna().astype(int).unique()
        if "most_recent_year" in features_df.columns else []
    )

    # Column renames: raw name → features CSV name
    RAW_RENAMES = {
    ...
    extra_frames = []
    for raw_path in sorted(RAW_DIR.glob("ncaaw_players_raw_*.csv")):
        m = re.search(r"_(\d{4})\.csv$", raw_path.name)
        if not m:
            continue
        year = int(m.group(1))
        ...
        try:
            ...
            print(f"  [WARN] Could not load {raw_path.name}: {e}")
            continue

        df = df.rename(columns=RAW_RENAMES)

        # Compute derived columns present in the features CSV
        if "ast_per_g" in df.columns and "tov_per_g" in df.columns:
            tov = pd.to_numeric(df["tov_per_g"], errors="coerce").replace(0, np.nan)
            df["ast_tov_ratio"] = (
                pd.to_numeric(df["ast_per_g"], errors="coerce") / tov
            )

        df["most_recent_year"] = year
    ...
    print(f"  Historical lookup pool: {len(historical)} rows from "
          f"{sorted(f['most_recent_year'].iloc[0] for f in extra_frames)} "
          f"(raw files not in features CSV)")

    # Merge opp_win_pct from team_opp_strength.csv for historical rows.
    # The raw files have team_id and season_year; team_opp_strength.csv covers
    # 2018-2025, so 2018-2021 draft class rows get proper SOS values here.
    opp_path = PROC_DIR / "team_opp_strength.csv"
    if opp_path.exists() and "team_id" in historical.columns:
        opp_df = pd.read_csv(opp_path)[["season_year", "team_id", "opp_win_pct"]]
        historical = historical.merge(
            opp_df, on=["season_year", "team_id"], how="left", suffixes=("", "_from_opp")
        )
        # Fill missing opp_win_pct (column may already exist with NaNs)
        if "opp_win_pct_from_opp" in historical.columns:
            mask = historical["opp_win_pct"].isna() if "opp_win_pct" in historical.columns else pd.Series(True, index=historical.index)
            historical["opp_win_pct"] = historical.get("opp_win_pct", pd.Series(dtype=float))
            historical.loc[mask, "opp_win_pct"] = historical.loc[mask, "opp_win_pct_from_opp"]
            historical = historical.drop(columns=["opp_win_pct_from_opp"])
        filled = historical["opp_win_pct"].notna().sum()
        print(f"  opp_win_pct merged for {filled}/{len(historical)} historical rows")

    combined = pd.concat([features_df, historical], ignore_index=True)
    return combined
...
def main():
    ...
    # Extend the lookup pool with raw files for historical years (2018–2021)
    # that were not included in the features CSV (which covers 2022+ only).
    lookup_pool = load_historical_features(prospects_df)
    train_df = match_ncaa_features(merged, lookup_pool)
